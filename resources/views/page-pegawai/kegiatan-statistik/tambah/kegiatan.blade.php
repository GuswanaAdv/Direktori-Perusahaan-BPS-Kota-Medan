...
@section('content')
    <div class="flex items-center justify-center py-8">
        <div class="card bg-white sm:w-3/4 shadow-xl w-full">
            ...
            <div class="card-body w-full text-left sm:px-20">
                ...
                <form class="mx-auto w-full" action="{{ route('proses-kegiatan-tambah') }}" method="POST" enctype="multipart/form-data">
                    @csrf
    ...
    <script>
        ...
        function getTommorowDate(){
            const today = new Date();
            today.setDate(today.getDate() + 30); // Menambahkan satu hari
            const yyyy = today.getFullYear();
            const mm = String(today.getMonth() + 1).padStart(2, '0'); // Tambah 1 karena bulan dimulai dari 0
            const dd = String(today.getDate()).padStart(2, '0');
            return `${yyyy}-${mm}-${dd}`;
        ...
        document.getElementById('waktu-selesai').value = getTommorowDate();
    </script>
@endsection
