...
class PerusahaanAprove implements WithHeadings, FromCollection, WithMapping, WithTitle
{
    ...
    public function map($row): array
    {
        return [
            ...
            'lattitude' => strval($row->lattitude),
            ...
            'kode_jenis_kepemilikan' => strval($row->kode_jenis_kepemilikan),
            'kode_bentuk_badan_usaha' => strval($row->kode_bentuk_badan_usaha),
            'kode_laporan_keuangan' => strval($row->kode_laporan_keuangan),
            'tahun_berdiri' => $row->tahun_berdiri,
            'tahun_mulai_beroperasi' => $row->tahun_mulai_beroperasi,
            'no_induk_berusaha' => $row->no_induk_berusaha,
            'kode_skala_usaha' => strval($row->kode_skala_usaha),
            'kode_jaringan_usaha' => strval($row->kode_jaringan_usaha),
            'kode_preferensi' => strval($row->kode_preferensi),
            ...
            'kode_jabatan_penanggungjawab' => strval($row->kode_jabatan_penanggungjawab),
            'nama_pemegang_saham' => $row->nama_pemegang_saham,
            'npwp_perusahaan' => $row->npwp_perusahaan,
            'kode_status_penanaman_modal' => strval($row->kode_status_penanaman_modal),
        ];
    }
